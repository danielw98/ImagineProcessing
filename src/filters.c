...
static PixelDouble Filters_Convolution(Image *image, Image *selection, double kernel[KERNEL_SIZE][KERNEL_SIZE], uint32_t row, uint32_t col) 
{   
	...
	for (uint32_t i = 0; i < KERNEL_SIZE; i++) 
		for (uint32_t j = 0; j < KERNEL_SIZE; j++)
		{
			...
			if(i+row >= selection->height || j+col >= selection->width)
				return (PixelDouble) {.r = -1, .g = -1, .b = -1};
...
static void Filters_ApplyKernel(Image *image, Image *selection, Filters *filter, FilterEnum filterType)
{
	// double pentru ca sa avem cat mai mare precizie
	double kernel[KERNEL_SIZE][KERNEL_SIZE];
	Filters_InitKernel(kernel, filter, filterType);
	// sigur sunt pixeli RGB
	PixelRGB_t **selectionPixels = (PixelRGB_t**) selection->pixels;
	// aplica kernelul curent pe portiunea de imagine admisa
	for(uint32_t i = 0; i < selection->height; i++)
	{
		for(uint32_t j = 0; j < selection->width; j++)
		{
			PixelDouble result = Filters_Convolution(image, selection, kernel, i, j);
			// skip peste pixelii care nu pot fi afectati de intergul kernel
			if(result.r == -1)
				continue;
			selectionPixels[i][j].r = (uint8_t) result.r;
			selectionPixels[i][j].g = (uint8_t) result.g;
			selectionPixels[i][j].b = (uint8_t) result.b;
			if(filterType == FILTER_EDGE_DETECTION)
			{
				selectionPixels[i][j].r = clamp(result.r);
				selectionPixels[i][j].g = clamp(result.g);
				selectionPixels[i][j].b = clamp(result.b);
			}
		}
	}
}
static void Filters_EdgeDetection(Filters *filters, Image *image, Image *selection)
{
	// sigur sunt pixeli RGB, deci functia nu verifica tipul imaginii
	Filters_ApplyKernel(image, selection, filters, FILTER_EDGE_DETECTION);
}
static void Filters_Sharpen(Filters *filters, Image *image, Image *selection)
{
	// sigur sunt pixeli RGB, deci functia nu verifica tipul imaginii
	Filters_ApplyKernel(image, selection, filters, FILTER_SHARPEN);
}
static void Filters_BoxBlur(Filters *filters, Image *image, Image *selection)
{
	// sigur sunt pixeli RGB, deci functia nu verifica tipul imaginii
	Filters_ApplyKernel(image, selection, filters, FILTER_BOX_BLUR);
}
static void Filters_GaussianBlur(Filters *filters, Image *image, Image *selection)
{
	// sigur sunt pixeli RGB, deci functia nu verifica tipul imaginii
	Filters_ApplyKernel(image, selection, filters, FILTER_GAUSSIAN_BLUR);
}
...
void Filters_Apply(Image *image, Image *selection, FilterEnum type)
{
	...
	{
		case FILTER_EDGE_DETECTION: {
			Filters_EdgeDetection(&filters, image, selection);
			break;
		}
		case FILTER_SHARPEN: {
			Filters_Sharpen(&filters, image, selection);
			break;
		}
		case FILTER_BOX_BLUR:{
			Filters_BoxBlur(&filters, image, selection);
			break;
		}
		case FILTER_GAUSSIAN_BLUR: {
            Filters_GaussianBlur(&filters, image, selection);
            break;
        }
        // filtrul e garantat unul din cele 4, altfel nu se apela functia
        default: {
