...
bool Image_Copy(Image *image, Image **selection, uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2)
{
    ...
    *selection = (Image*) malloc(sizeof(Image));
    ...
    selection[0]->isEqualized = image->isEqualized;
...
bool Image_IsImage(const char *fileName)
{
    union MagicBytes magicBytes = readMagicBytes(fileName);
    // 5032
    if(((magicBytes.sh != 0x5032) && (magicBytes.sh != 0x5033) && (magicBytes.sh != 0x5035) && (magicBytes.sh != 0x5036)) && 
       ((magicBytes.sh != 0x3250) && (magicBytes.sh != 0x3350) && (magicBytes.sh != 0x3550) && (magicBytes.sh != 0x3650)))
    {
        return false;
    }
    return true;
...
static Image* Image_LoadP5(const char *fileName)
{
    ...
    fread(&magic_bytes, sizeof(uint16_t), 1, fp);
    fread(&endl, sizeof(char), 1, fp);
    skip_comments(fp);
    // alocare imagine
    Image *image = (Image*) malloc(sizeof(Image));
    image->width = image->height = 0;
