...
void equalization_histogram(Image *image, Histogram *histogram)
{
	...
	for(uint32_t i = 0; i < image->height; i++)
	{
		for(uint32_t j = 0; j < image->width; j++)
		{
			...
			long long int new_pixel = 255 * sum_of_histogram[current_pixel];
			...
			pixels[i][j] = (new_pixel > 255*area) ? 255 : (PixelGrayScale_t) round(new_pixel/area);
		}
	}
	// am terminat treaba
